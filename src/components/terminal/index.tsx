...
export function Terminal() {
  ...
  return (
    <div className="w-full max-w-5xl h-[80vh] md:h-[85vh] flex flex-col bg-glass-surface backdrop-blur-2xl border border-glass-border rounded-2xl relative z-10 overflow-hidden shadow-2xl shadow-black/50">
          ...
          <div className="flex gap-3 overflow-x-auto pt-1 w-full md:w-auto pb-2 md:pb-0 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            {QUICK_COMMANDS.map((cmd) => (
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-semibold text-white/90 hover:text-white shrink-0 bg-white/5 border border-white/10 backdrop-blur-sm transition-all duration-300 hover:bg-white/15 hover:border-white/30 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-black/20 cursor-pointer"
              >
